...
def start_training(vae, image_translator, ctf, grid, gmm_repr, optimizer, dataset, N_epochs, batch_size, experiment_settings, scheduler, 
    ...
    for epoch in range(N_epochs):
        ...
        for batch_num, (indexes, batch_images, batch_poses, batch_poses_translation, _) in enumerate(data_loader):
            ...
            posed_predicted_structures, rotated_precisions = renderer.rotate_structure(predicted_structures, batch_poses)
            predicted_images = renderer.project_non_diagonal_gaussian(posed_predicted_structures[:, :, 2], rotated_precisions[:, :, :2, :2], Gauss_amplitudes, grid)
